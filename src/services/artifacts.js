/**
 * @file        packages/coordinator/src/services/artifacts.js
 * @description Job artifact storage: saves uploaded files and generates download links
 *
...
function createArtifactsService(db, { config }){
  ...
  function sign(artifactId, ttlHours = config.artifacts.linkTtlHours){
    const expiresAt = Date.now() + ttlHours * 3600 * 1000,
      payload = `${artifactId}.${expiresAt}`,
      mac = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
    return `${payload}.${mac}`;
  }

  function verify(token){
    const [artifactId, expiresAt, mac] = String(token).split('.');
    if (!artifactId || !expiresAt || !mac){
      throw invalidLink();
    }
    const payload = `${artifactId}.${expiresAt}`,
      expectedMac = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
    if (!crypto.timingSafeEqual(Buffer.from(mac), Buffer.from(expectedMac))){
      throw invalidLink();
    }
    if (Date.now() > Number(expiresAt)){
      throw invalidLink('Link expired');
    }
